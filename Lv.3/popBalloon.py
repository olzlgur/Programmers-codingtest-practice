...
def solution(a):
    answer = 0
    leftMin, rightMin = -1000001, -1000001
    leftMin, rightMin = a[0], a[-1]
    
    # 각 숫자의 왼쪽과 오른쪽의 가장 큰 수를 담을 리스트
    leftMinList, rightMinList = [0] * len(a), [0] * len(a)
    ...
    for i in range(len(a)-1, -1, -1):
        # 현재 위치의 요소와 오른쪽에서 가장 큰 값 비교
        if rightMin > a[i]:
            # 클 경우 max값 초기화
            rightMin = a[i]
        # 현재 위치의 오른쪽에서 가장 큰 값 저장
        rightMinList[i] = rightMin
    
    # 동일
    for i in range(1, len(a)):
        if leftMin > a[i]:
            leftMin = a[i]
        leftMinList[i] = leftMin
    
    # 해당 요소와 왼쪽에서 가장 큰 수 혹은 오른쪽에서 가장 큰 수가 
    for i in range(len(a)):
        if leftMinList[i] == a[i] or rightMinList[i] == a[i]:
            answer += 1
